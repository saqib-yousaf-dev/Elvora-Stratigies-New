...
function FloatingChatbot() {
  return (
    ...
    >
      ...
      <MessageSquare size={26} className="text-white relative z-10 group-hover:-rotate-12 transition-transform duration-300" />
    </button>
  );
}
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between h-20">
        <Link href="/">
          <span className="flex items-center gap-3 cursor-pointer group">
            <img src={LOGO_TRANSPARENT} alt="Elvora Strategies" className="h-12 w-auto transition-transform duration-500 group-hover:scale-105" />
            <span className="font-bold text-xl tracking-tight text-gray-900" style={{ fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          ...
          >
            ...
            <div className={`absolute top-full -left-4 pt-4 transition-all duration-300 ${servicesOpen ? 'opacity-100 visible translate-y-0' : 'opacity-0 invisible translate-y-4'}`}>
              <div className="bg-white/95 backdrop-blur-xl border shadow-[0_20px_40px_rgba(0,0,0,0.08)] rounded-2xl w-64 p-2 flex flex-col gap-1" style={{ borderColor: "#e0f2fe" }}>
                {SERVICES.map((s) => (
      ...
      {menuOpen && (
        <div className="lg:hidden bg-white border-t border-sky-100 px-6 py-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto shadow-2xl">
          <p className="text-xs font-bold uppercase tracking-widest text-sky-600 mb-2">Services</p>
          {SERVICES.map((s) => (
            <Link key={s.title} href={s.link}>
              <span onClick={() => setMenuOpen(false)} className="text-gray-600 font-medium py-2 pl-4 border-b border-gray-50 block cursor-pointer">
                {s.title}
              </span>
            </Link>
          ))}
          <p className="text-xs font-bold uppercase tracking-widest text-sky-600 mt-4 mb-2">Menu</p>
          {navLinks.map((l) => (
...
function Hero() {
  return (
    <section className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #f7fcff 0%, #e0f2fe 58%, #fffbeb 100%)" }}>
      <div className="absolute top-0 right-0 w-1/2 h-full hidden lg:block opacity-60 pointer-events-none" style={{ background: "radial-gradient(circle at 70% 30%, rgba(245,166,35,0.15), transparent 40%), radial-gradient(circle at 40% 60%, rgba(14,165,233,0.15), transparent 40%)" }} />
        ...
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-12 lg:gap-20 items-center">
          ...
          <div className="max-w-3xl relative z-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-10">
              <button onClick={openChatbot} className="group inline-flex items-center justify-center gap-2 font-bold text-sm px-8 py-4 transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(245,166,35,0.4)] rounded-xl" style={{ background: "#F5A623", color: "#1a1a1a" }}>
                Try Pulse <ArrowRight size={18} className="transition-transform duration-300 group-hover:translate-x-1" />
              </button>
              <a href="#pricing" className="group inline-flex items-center justify-center gap-2 border-2 font-bold text-sm px-8 py-4 text-slate-700 transition-all duration-300 hover:bg-white hover:border-sky-400 hover:shadow-lg rounded-xl" style={{ borderColor: "#bae6fd" }}>
                Explore plans <ChevronRight size={18} className="transition-transform duration-300 group-hover:translate-x-1" />
              </a>
            </div>
          ...
          <div className="relative group perspective-1000">
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] border border-white/80 bg-white shadow-[0_30px_90px_rgba(14,165,233,0.18)] transition-transform duration-700 hover:-translate-y-2">
              ...
              <div className="relative h-56 overflow-hidden bg-slate-950">
                <video autoPlay muted loop playsInline className="w-full h-full object-cover opacity-45" src={HERO_VIDEO} />
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-6 pb-3 border-b border-sky-200/60">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] font-medium text-slate-600">Your AI receptionist</p>
                    </div>
                    <span className="ml-auto text-xs font-bold px-3 py-1 rounded-full shadow-sm" style={{ background: "#dcfce7", color: "#15803d" }}>Online</span>
                  </div>

                  <div className="space-y-4 text-sm leading-relaxed">
                    <div className="max-w-[88%] p-4 rounded-2xl rounded-tl-sm shadow-md font-medium border border-sky-200 bg-white/95" style={{ color: "#0369a1" }}>
                      Hi, I’m Pulse. How can I help with your enquiry today?
                    </div>
                    <div className="ml-auto max-w-[82%] p-4 rounded-2xl rounded-tr-sm border border-slate-200 shadow-sm font-medium bg-white/95" style={{ color: "#334155" }}>
                      I’d like to understand whether your service is right for me.
                    </div>
                    <div className="max-w-[88%] p-4 rounded-2xl rounded-tl-sm shadow-md font-medium border border-sky-200 bg-white/95" style={{ color: "#0369a1" }}>
                      Of course. I can explain how it works and make sure the right person receives your enquiry.
                    </div>
                  </div>
                </div>
...
function ServiceCard({ s, index }: { s: typeof SERVICES[0]; index: number }) {
  ...
  return (
    <Link href={s.link}>
      ...
      >
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none bg-gradient-to-br ${isBlue ? 'from-sky-50/50 to-transparent' : 'from-amber-50/50 to-transparent'}`} />
        ...
        <div 
          ...
          style={{ background: isBlue ? "linear-gradient(90deg, #0ea5e9, #7dd3fc)" : "linear-gradient(90deg, #F5A623, #fde047)" }} 
        />
        ...
        <div className="p-10 md:p-12 flex-1 flex flex-col relative z-10">
          <div className="flex justify-between items-start mb-10 flex-wrap gap-4">
            ...
            >
              <Icon size={30} style={{ color: isBlue ? "#0284c7" : "#d97706" }} className="transition-colors duration-300" />
            </div>
          ...
          >
            <span className="relative overflow-hidden flex items-center gap-3 group-hover:gap-5 transition-all duration-300">
              Explore Feature 
              <span className="flex items-center justify-center w-8 h-8 rounded-full transition-all duration-300 group-hover:bg-current group-hover:text-white" style={{ background: isBlue ? "#e0f2fe" : "#fef3c7" }}>
                <ArrowRight size={16} />
              </span>
            </span>
          </div>
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        <div className="grid lg:grid-cols-[1fr_1fr] gap-10 lg:gap-20 items-end mb-24">
          <div>
            ...
            <h2 className="text-5xl lg:text-6xl font-bold text-gray-900 leading-[1.1]" style={{ fontFamily: "'Playfair Display', serif" }}>
          ...
          <div className="lg:border-l-4 lg:pl-10" style={{ borderColor: "#bae6fd" }}>
            ...
            <p className="text-gray-500 text-xl leading-relaxed">
              Pulse is trained around your business and can take the first conversation from question to informed handover. Select a capability to dive deeper.
            </p>
          </div>
        </div>
...
function Industries() {
  return (
    <section id="industries" className="py-32 relative overflow-hidden bg-white">
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-10">
        <div className="mb-20 text-center max-w-3xl mx-auto">
          ...
          <h2 className="text-4xl lg:text-6xl font-bold text-gray-900 leading-tight" style={{ fontFamily: "'Playfair Display', serif" }}>
          ...
          <p className="text-gray-500 mt-8 text-lg md:text-xl">
            Pulse is engineered for high-intent, high-value service sectors where every delayed response equals lost revenue.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {INDUSTRIES.map((ind) => {
            const Icon = ind.icon;
            return (
              <div key={ind.name} className="group relative p-8 rounded-3xl bg-white border border-sky-100 overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(14,165,233,0.1)] cursor-default">
                ...
                <div className="relative z-10">
                  ...
                  <h3 className="text-2xl font-bold text-gray-900 mb-4 transition-colors duration-300 group-hover:text-sky-800" style={{ fontFamily: "'Playfair Display', serif" }}>
                    {ind.name}
                  </h3>
        ...
        <div className="mt-20 p-10 lg:p-14 rounded-3xl flex flex-col lg:flex-row items-center justify-between gap-8 border border-sky-100 bg-white shadow-xl shadow-sky-900/5 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-2xl lg:text-3xl font-bold text-gray-900 mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>
          ...
          <div className="relative group">
            <div className="overflow-hidden aspect-[4/3] bg-sky-100 rounded-3xl border border-sky-200 relative shadow-inner">
              ...
              <div className="absolute top-6 left-6 bg-white/90 backdrop-blur-md px-4 py-2 rounded-full shadow-lg flex items-center gap-2 border border-white/50">
                ...
                <span className="text-xs font-bold text-slate-700 tracking-wider uppercase">AI Models Live</span>
              </div>
            </div>
            ...
            >
              <div className="text-5xl font-black text-gray-900 mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>
                100%
              </div>
              <div className="text-gray-600 text-sm font-medium leading-relaxed">Bespoke — every AI assistant is built and trained exclusively for your business.</div>
            </div>
          </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm leading-relaxed">
                  You are always welcome to call us. We use <strong>Pulse</strong> as our first point of contact because it is trained on our business and can answer most questions immediately, day or night. If you would prefer to speak with our team, call <a href="tel:+441615150022" className="font-semibold underline" style={{ color: "#0284c7" }}>01615 150022</a>.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="relative z-10">
                <div className="flex items-center justify-between gap-3 mb-10 border-b border-gray-100 pb-6">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest text-gray-400">{t.location}</span>
                </div>
                ...
                <div className="inline-flex items-center gap-2 text-sm font-bold bg-sky-50 px-5 py-3 rounded-xl border border-sky-100" style={{ color: "#0ea5e9" }}>
    ...
    <section id="pricing" className="py-32 bg-white relative">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-xs font-black tracking-widest uppercase mb-4 block text-sky-500">Simple, Transparent Pricing</span>
          ...
          <p className="text-gray-500 text-lg md:text-xl">
            Every plan includes a bespoke receptionist trained on your business. Start where your enquiry volume is today, then grow when you need to.
          </p>
        </div>
        ...
        <div className="flex justify-center mb-16">
          <div className="bg-slate-100 p-1.5 rounded-full inline-flex items-center relative shadow-inner border border-slate-200">
            ...
            >
              Annual
              <span className="absolute -top-3 -right-3 px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-black bg-amber-400 text-slate-900 shadow-sm border border-amber-200 transform rotate-3">Save 10%</span>
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 items-stretch">
          {PRICING_TIERS.map((tier) => {
            ...
            <div
              ...
              style={{
                borderTop: isHighlighted ? "4px solid #F5A623" : "4px solid #0ea5e9",
              }}
            >
              {isHighlighted && (
              ...
              {isHighlighted && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-4 py-1.5 text-xs font-black tracking-widest uppercase rounded-full shadow-lg" style={{ background: "#F5A623", color: "#1a1a1a" }}>
                  <Star size={12} fill="#1a1a1a" /> Most Popular
                </div>
              )}
              
              <span className={`text-xs font-black tracking-widest uppercase mb-4 block ${isHighlighted ? 'text-sky-200' : 'text-sky-500'}`}>
                {tier.name}
              </span>
              
              <div className="mb-2 flex items-end gap-1 relative z-10">
                <span className={`text-5xl font-black ${isHighlighted ? 'text-white drop-shadow-sm' : 'text-slate-900'}`} style={{ fontFamily: "'Playfair Display', serif" }}>
                  {displayPrice}
                </span>
                {!(tier as any).isCustom && (
                  <span className={`text-base mb-2 font-bold ${isHighlighted ? 'text-sky-200' : 'text-slate-400'}`}>/mo</span>
                )}
              </div>
              
              {annual && !(tier as any).isCustom && (tier as any).annualMonthly && (
                <p className={`text-xs font-bold ${isHighlighted ? 'text-amber-300' : 'text-sky-600'}`}>
                  Billed as {(tier as any).annualPrice}/year
                </p>
              )}
              {!(tier as any).isCustom && (
                <p className={`text-xs font-medium mt-1 ${isHighlighted ? 'text-sky-200' : 'text-slate-500'}`}>
                  + {(tier as any).setupFee} setup fee
                </p>
              )}
              
              <p className={`text-sm mb-10 mt-6 font-medium ${isHighlighted ? 'text-sky-100' : 'text-slate-600'}`}>{tier.tagline}</p>
              
              <ul className="flex flex-col gap-4 mb-10 flex-1 relative z-10">
                {tier.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-sm">
                    <CheckCircle size={18} className="flex-shrink-0 mt-0.5" style={{ color: isHighlighted ? "#F5A623" : "#0ea5e9" }} />
                    <span className={`font-medium leading-relaxed ${isHighlighted ? 'text-white' : 'text-slate-700'}`}>{f}</span>
                  </li>
                ))}
              </ul>
              ...
              <div className="flex flex-col gap-3 relative z-10 mt-auto">
                ...
                >
                  {(tier as any).isCustom ? "Tell Us What You Need" : "Ask Pulse about this plan"}
                </button>
                <Link href={(tier as any).packageUrl}>
                  <span className={`block w-full py-3 text-center text-xs font-bold uppercase tracking-widest cursor-pointer hover:underline transition-colors ${isHighlighted ? 'text-amber-300' : 'text-sky-600'}`}>
                    Learn More →
                  </span>
                </Link>
              </div>
        ...
        <div className="mt-16 text-center max-w-2xl mx-auto p-8 rounded-3xl bg-slate-50 border border-slate-100">
          ...
          <p className="text-gray-500 text-sm">
            ...
            <button onClick={openChatbot} className="underline font-bold transition-colors" style={{ color: "#0284c7" }}>
              book a demo
            </button>{" "}
...
function Process() {
  ...
  const targetMonthly = Math.round(enquiries * dealValue * 0.30);
  ...
  return (
    <section id="process" className="py-32 border-y relative overflow-hidden" style={{ background: "#f8fbff", borderColor: "#e0f2fe" }}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-8" style={{ fontFamily: "'Playfair Display', serif" }}>
            ...
            <div className="flex flex-col gap-8 mb-10 relative">
              ...
              {PROCESS.map((p, index) => (
                <div key={p.step} className="group relative flex gap-6 items-start transition-all duration-300 hover:translate-x-2 cursor-default">
                  <div className="relative z-10 flex-shrink-0 w-12 h-12 rounded-full flex items-center justify-center font-black text-sm shadow-md transition-colors duration-300 group-hover:bg-amber-400 group-hover:text-white" style={{ background: "#ffffff", color: "#F5A623", border: "2px solid #Fef3c7" }}>
                    {p.step}
                  </div>
                  <div className="pt-2">
                    <h4 className="font-bold text-gray-900 text-xl mb-2 transition-colors duration-300 group-hover:text-amber-600" style={{ fontFamily: "'Playfair Display', serif" }}>
                      {p.title}
                    </h4>
          ...
          <div className="relative">
            ...
            <div className="relative bg-white/90 backdrop-blur-xl border border-white shadow-2xl rounded-3xl overflow-hidden">
              <div className="p-8 md:p-10 border-b border-gray-100">
                ...
                <p className="text-gray-500 text-sm mb-10 font-medium">Model the revenue Pulse can recover from your ad traffic.</p>

                <div className="space-y-8">
                  <div>
                    <div className="flex justify-between mb-3">
                      <label className="text-sm font-bold text-gray-700">Monthly Missed/Delayed Enquiries</label>
                      <span className="text-sm font-bold text-sky-600">{enquiries}</span>
                    </div>
                    <input
                      type="range"
                      min={0}
                  ...
                  <div>
                    <div className="flex justify-between mb-3">
                      <label className="text-sm font-bold text-gray-700">Average Deal Value</label>
                      <span className="text-sm font-bold text-amber-500">£{dealValue.toLocaleString()}</span>
                    </div>
                    <input
                      type="range"
                      min={500}
              ...
              <div className="p-10 text-center relative overflow-hidden bg-slate-900">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-6xl font-black mb-3 text-white drop-shadow-md flex justify-center items-baseline gap-1" style={{ fontFamily: "'Playfair Display', serif" }}>
                  ...
                  <p className="text-xs text-slate-400 mt-6 font-medium">Based on a standard 30% closing rate.</p>
                </div>
              </div>
...
function WhiteLabelling() {
  return (
    <section id="white-labelling" className="py-32 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #f0f9ff 0%, #ffffff 100%)" }}>
              ...
              <div key={i} className="group p-6 rounded-2xl bg-white border border-sky-100 shadow-sm transition-all duration-300 hover:shadow-md hover:border-amber-200 hover:-translate-y-1">
                <div className="flex items-start gap-5">
                  <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5 bg-sky-50 transition-colors duration-300 group-hover:bg-amber-100">
                    <CheckCircle size={20} className="text-sky-500 transition-colors duration-300 group-hover:text-amber-600" />
                  </div>
                  <div>
                    <p className="font-bold text-lg text-gray-900 mb-2 transition-colors duration-300 group-hover:text-amber-600">{feature.title}</p>
          ...
          <div className="relative perspective-1000">
            ...
            <div className="relative bg-white/60 backdrop-blur-2xl border border-white shadow-2xl rounded-3xl p-10 overflow-hidden">
              <div className="flex items-center gap-2 mb-8 pb-6 border-b border-slate-200/50">
                ...
                <span className="ml-2 text-xs font-bold tracking-widest uppercase text-slate-400">Partner Portal</span>
              </div>

              <h3 className="text-3xl font-bold text-gray-900 mb-8" style={{ fontFamily: "'Playfair Display', serif" }}>Perfect For</h3>
              ...
              <ul className="space-y-5 mb-10 relative z-10">
                {[
                  ...
                  <li key={i} className="text-slate-700 font-medium flex items-center gap-4 text-base">
              ...
              <Globe size={200} className="absolute -right-10 -bottom-10 text-sky-100 opacity-50 z-0 pointer-events-none" />
            </div>
          </div>
        </div>
      </div>
    ...
    <section className="py-32 relative overflow-hidden bg-white">
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-10 relative z-10">
        <div className="bg-white border border-sky-100 rounded-3xl p-10 md:p-16 text-center shadow-[0_20px_60px_rgba(14,165,233,0.08)] relative overflow-hidden">
          ...
          <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 leading-tight mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
...
function DataSecurity() {
  return (
    <section id="security" className="py-32 relative bg-slate-50 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 relative z-10">
        <div className="mb-20 text-center max-w-3xl mx-auto">
          ...
          <p className="text-gray-500 text-lg md:text-xl">
            We take data protection seriously. Here is exactly what we collect, how we protect it, and what rights you have. No compromises.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="p-10 border border-sky-100 rounded-3xl bg-white shadow-xl shadow-sky-900/5 transition-transform duration-500 hover:-translate-y-2 relative overflow-hidden group">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>What We Collect</h3>
            <p className="text-gray-500 text-base mb-8">When a visitor interacts with Pulse on your website, we capture:</p>
            <ul className="flex flex-col gap-5 mb-8">
              {["Name, email, phone number, a short enquiry summary, and the full conversation transcript", "Date and time captured", "Browser language and display preferences"].map((item, i) => (
                <li key={i} className="flex items-start gap-4 text-sm font-medium text-gray-700">
                  <CheckCircle size={20} className="flex-shrink-0 text-sky-500" />
          ...
          <div className="p-10 border border-amber-100 rounded-3xl bg-white shadow-xl shadow-amber-900/5 transition-transform duration-500 hover:-translate-y-2 relative overflow-hidden group">
            ...
            <h3 className="text-2xl font-bold text-gray-900 mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>How It's Protected</h3>
        ...
        <div className="mt-12 flex justify-center relative z-10">
          <div className="inline-flex items-center gap-3 bg-white border border-sky-100 px-8 py-4 rounded-full shadow-md transition-transform hover:scale-105">
            ...
            <span className="text-sm md:text-base font-bold text-gray-800 tracking-wide uppercase">100% GDPR Compliant & Fully Insured</span>
          </div>
        </div>
      </div>
    </section>
...
function Contact() {
  return (
    <section id="contact" aria-hidden="true" style={{ background: "#ffffff", borderColor: "#fffafa", height: "8px" }}></section>
  );
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <img src={LOGO_TRANSPARENT} alt="Elvora Strategies logo" className="h-10 w-auto" />
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm font-medium text-slate-600">
              <li><a href="#services" className="hover:text-amber-500 transition-colors cursor-pointer">Capabilities</a></li>
              <li><a href="#industries" className="hover:text-amber-500 transition-colors cursor-pointer">Industries</a></li>
              <li><a href="#about" className="hover:text-amber-500 transition-colors cursor-pointer">About Us</a></li>
              <li><a href="#pricing" className="hover:text-amber-500 transition-colors cursor-pointer">Pricing Tiers</a></li>
            </ul>
          </div>
