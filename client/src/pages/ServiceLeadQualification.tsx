...
import { CheckCircle, ArrowLeft, MessageSquare, Zap, Filter, FileText, Send, ArrowRight } from "lucide-react";
    ...
    <div className="min-h-screen" style={{ background: "#f8fbff" }}>
      ...
      <section className="relative py-24 px-6 lg:px-10 overflow-hidden" style={{ background: "linear-gradient(135deg, #0ea5e9 0%, #38bdf8 50%, #7dd3fc 100%)" }}>
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-5xl lg:text-7xl font-bold text-white mb-6 leading-tight drop-shadow-sm" style={{ fontFamily: "'Playfair Display', serif" }}>
            Lead Qualification <br/><span style={{ color: "#F5A623" }}>&amp; Delivery</span>
          </h1>
          <p className="text-white/90 text-xl max-w-2xl mx-auto leading-relaxed mb-10">
            Pulse doesn't just take a message and hope for the best. It asks the right questions and delivers a ready-to-act lead straight to your team.
          </p>
          <button onClick={openChatbot} className="group relative inline-flex items-center justify-center gap-2 font-bold text-sm px-8 py-4 transition-all duration-300 hover:scale-105 hover:shadow-[0_0_30px_rgba(245,166,35,0.4)] overflow-hidden rounded-sm" style={{ background: "#F5A623", color: "#1a1a1a" }}>
            ...
            <span className="relative flex items-center gap-2">
              <MessageSquare size={18} className="transition-transform duration-300 group-hover:rotate-12" /> Experience the Flow
            </span>
          </button>
        </div>
      </section>
        ...
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-20">
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-6" style={{ fontFamily: "'Playfair Display', serif" }}>
              Stop chasing <span className="relative inline-block">unqualified traffic.<span className="absolute -bottom-1 left-0 right-0 h-1 bg-amber-400 opacity-60 rounded-full" /></span>
            </h2>
            <p className="text-gray-500 max-w-2xl mx-auto text-lg leading-relaxed">
              We engineer your AI assistant to act as a rigorous first filter, ensuring your human team only spends time closing real prospects.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {FEATURES.map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.title} className="group bg-white p-8 border rounded-lg transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(245,166,35,0.12)] cursor-default" style={{ borderTop: "3px solid #F5A623", borderColor: "#e0f2fe" }}>
                  <div className="flex items-start gap-5">
                    <div className="w-14 h-14 flex-shrink-0 flex items-center justify-center rounded-xl transition-all duration-500 group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-md" style={{ background: "#fef3c7" }}>
                      <Icon size={26} style={{ color: "#d97706" }} className="transition-colors duration-300 group-hover:text-amber-700" />
                    </div>
                    <div>
                      <h3 className="font-bold text-gray-900 mb-3 text-xl tracking-tight transition-colors duration-300 group-hover:text-amber-600">{f.title}</h3>
      ...
      <section className="relative py-28 px-6 lg:px-10 overflow-hidden" style={{ background: "#0ea5e9" }}>
        ...
        <div className="relative max-w-2xl mx-auto text-center">
          <span className="text-xs font-bold tracking-widest uppercase mb-5 block text-sky-200">Increase Your Closing Rate</span>
          ...
          <p className="text-white/90 mb-12 text-lg font-medium">Let Pulse handle the qualifying questions so you can focus on closing the deal.</p>
          <div className="flex flex-col sm:flex-row justify-center gap-5">
            ...
            >
              <MessageSquare size={18} className="transition-transform duration-300 group-hover:-translate-y-1" /> Test the Qualification Flow
            </button>
            <Link href="/#pricing">
