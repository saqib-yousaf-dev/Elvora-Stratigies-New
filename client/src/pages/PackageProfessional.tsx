...
import { CheckCircle, ArrowLeft, MessageSquare, Star, Mail, Linkedin, Phone, Zap } from "lucide-react";
...
export default function PackageProfessional() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Elite Hero Section */}
      <section className="relative py-24 px-6 lg:px-10 overflow-hidden" style={{ background: "linear-gradient(135deg, #0284c7 0%, #0ea5e9 50%, #38bdf8 100%)" }}>
        ...
        <div className="relative max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 mb-6 text-xs font-bold tracking-widest uppercase rounded-full shadow-md" style={{ background: "#F5A623", color: "#1a1a1a" }}>
            <Star size={12} fill="#1a1a1a" /> Most Popular Tier
          </div>
          <span className="text-xs font-bold tracking-widest uppercase mb-4 block text-white/70">Professional Plan</span>
          <h1 className="text-6xl lg:text-7xl font-bold text-white mb-4 tracking-tight drop-shadow-sm" style={{ fontFamily: "'Playfair Display', serif" }}>
            £149 <span className="text-3xl font-normal text-white/80">/ month</span>
          </h1>
          <p className="text-white/80 text-lg mb-6 font-medium">or <strong className="text-white">£1,609 / year</strong> — save 10%</p>
          <p className="text-white/90 text-xl max-w-2xl mx-auto leading-relaxed mb-10">
            Ideal for busy practices that need CRM integration, live appointment booking, and industry-specific qualification — fully automated.
          </p>
          <button 
            onClick={openChatbot} 
          ...
          >
            <MessageSquare size={18} className="transition-transform duration-300 group-hover:rotate-12" /> Ask Pulse About Professional
          </button>
        </div>
      </section>
      ...
      <section className="py-28 px-6 lg:px-10 relative">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <span className="text-xs font-black tracking-widest uppercase mb-3 block text-sky-500">Advanced Capabilities</span>
            <h2 className="text-4xl lg:text-5xl font-bold text-gray-900" style={{ fontFamily: "'Playfair Display', serif" }}>
              Everything in the <span className="text-sky-600">Professional Plan</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {FEATURES.map((f, i) => (
              <div 
                ...
                style={{ borderTop: "4px solid #0284c7", borderColor: "#e0f2fe" }}
              >
      ...
      <section className="py-16 px-6 lg:px-10" style={{ background: "#f0f9ff" }}>
        <div className="max-w-4xl mx-auto flex flex-col sm:flex-row gap-4 justify-between items-center text-center">
          <Link href="/packages/starter">
            <span className="font-bold text-sm cursor-pointer hover:underline text-sky-600">← Explore Starter Plan</span>
          </Link>
      ...
      <section className="relative py-28 px-6 lg:px-10 overflow-hidden" style={{ background: "#0284c7" }}>
        <div className="relative max-w-2xl mx-auto text-center">
          <span className="text-xs font-bold tracking-widest uppercase mb-4 block text-sky-200">Automate Your Intake</span>
          ...
          <p className="text-white/90 mb-10 text-lg font-medium">Your fully integrated AI receptionist can be live and syncing to your CRM in under 2 weeks.</p>
          <button
            onClick={openChatbot}
