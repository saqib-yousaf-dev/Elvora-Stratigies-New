...
function renderMarkdown(content: string): string {
  return content
    ...
    .replace(
      /^## (.+)$/gm,
      '<h2 class="text-3xl font-bold text-gray-900 mt-12 mb-6" style="font-family: \'Playfair Display\', serif">$1</h2>'
    )
    .replace(
      /\*\*(.+?)\*\*/g,
    ...
    .replace(
      /^- (.+)$/gm,
      '<li class="text-gray-600 leading-relaxed mb-2 ml-4 list-disc">$1</li>'
    )
    .replace(/(<li[\s\S]+?<\/li>)/g, '<ul class="mb-6 space-y-2">$1</ul>')
    .replace(
      /^(\d+)\. (.+)$/gm,
      '<li class="text-gray-600 leading-relaxed mb-3 ml-4 list-decimal"><strong class="text-gray-900">$2</strong></li>'
    )
    .replace(
      /\n\n/g,
      '</p><p class="text-gray-600 text-lg leading-relaxed mb-6">'
    )
    .replace(/^(?!<[hul])(.+)$/gm, line =>
      line.startsWith("<")
        ? line
        : `<p class="text-gray-600 text-lg leading-relaxed mb-6">${line}</p>`
    );
}
...
export default function BlogPost() {
  ...
  if (!post) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div
          className="text-center p-8 bg-white border rounded-3xl shadow-sm"
        ...
        >
          ...
          <Link href="/blog" className="text-sky-600 hover:underline font-bold">
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* Elite Hero Section */}
      <section
        className="relative py-24 px-6 lg:px-10 overflow-hidden text-white"
        style={{
          background:
      ...
      >
        ...
        <div className="relative max-w-3xl mx-auto">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 text-white/70 hover:text-white text-sm mb-8 transition-colors font-medium"
          >
          ...
          <h1
            className="text-4xl lg:text-6xl font-bold leading-tight mb-6 tracking-tight drop-shadow-sm"
            style={{ fontFamily: "'Playfair Display', serif" }}
          >
            {post.title}
          </h1>
      ...
      <article className="py-20 px-6 lg:px-10">
        <div
          className="max-w-3xl mx-auto bg-white p-10 lg:p-16 border rounded-[2.5rem] shadow-xl shadow-sky-900/5"
          style={{ borderColor: "#e0f2fe" }}
        >
          <p
            className="text-xl text-sky-900 leading-relaxed mb-10 font-medium border-l-4 pl-6 py-2 bg-sky-50/50 rounded-r-2xl"
      ...
      >
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-white/80 mb-8 text-lg">
            Book a live demo and see exactly what a bespoke AI receptionist
